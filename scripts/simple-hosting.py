...
class SimpleHostingManager:
    ...
    def run_command(
        self, command, require_root=False, capture_output=True, show_output=False
    ):
    ...
    def create_directory_with_permissions(
        self, directory, owner_user="www-data", mode=0o755
    ):
    ...
    def ensure_nginx_installed(self):
        ...
        try:
            if not self.check_nginx_installation():
                ...
                subprocess.run(
                    ["sudo", "systemctl", "enable", "nginx"], capture_output=True
                )
                subprocess.run(
                    ["sudo", "systemctl", "start", "nginx"], capture_output=True
                )
    ...
    def test_nginx_config_safe(self):
        ...
        try:
            ...
            if nginx_binary == "nginx":
                ...
                result = subprocess.run(
                    [nginx_binary, "-t"], capture_output=True, text=True
                )
            ...
            if (
                ...
                if "syntax is ok" in result.stderr:
                    print(
                        "   ✅ Nginx syntax check: PASSED (ignoring file system issues)"
                    )
                    return True
    ...
    def create_readonly_process_manager(
        self, site_name, final_dir, app_port, start_command=None
    ):
        ...
        try:
            ...
            if start_command:
                ...
                if os.path.exists(
                    os.path.join(final_dir, "next.config.mjs")
                ) or os.path.exists(os.path.join(final_dir, "next.config.js")):
                    # Use direct binary path for Next.js to avoid corruption
                    exec_command = "node_modules/next/dist/bin/next start"
                    print(
                        f"   🔍 Detected Next.js app, using direct binary: {exec_command}"
                    )
                    ...
                    try:
                        with open(os.path.join(final_dir, "package.json"), "r") as f:
                            ...
                            if "next" in dependencies:
                                exec_command = "node_modules/next/dist/bin/next start"
                                print(
                                    f"   🔍 Next.js in dependencies, using direct binary: {exec_command}"
                                )
...
"""
            ...
            test_result = subprocess.run(
                [script_path, "start"], capture_output=True, text=True
            )
            ...
            if test_result.returncode == 0:
                ...
                print(
                    f"   ❌ Read-only process manager test failed: {test_result.stderr}"
                )
    ...
    def get_readonly_app_status(self, site_name):
        ...
        try:
            ...
            if os.path.exists(control_script):
                result = subprocess.run(
                    [control_script, "status"], capture_output=True, text=True
                )
    ...
    def stop_readonly_app(self, site_name):
        ...
        try:
            ...
            if os.path.exists(control_script):
                result = subprocess.run(
                    [control_script, "stop"], capture_output=True, text=True
                )
                return result.returncode == 0
            return False
        except:
    ...
    def start_readonly_app(self, site_name):
        ...
        try:
            ...
            if os.path.exists(control_script):
                result = subprocess.run(
                    [control_script, "start"], capture_output=True, text=True
                )
                return result.returncode == 0
            return False
        except:
    ...
    def create_systemd_app_service(self, site_name, final_dir, app_port):
        ...
        try:
            ...
            if self.readonly_filesystem:
                return self.create_readonly_process_manager(
                    site_name, final_dir, app_port
                )
            ...
            if os.path.exists(
                ...
                if os.path.exists(os.path.join(final_dir, ".next")):
                    ...
                    if os.path.exists(server_dir):
                        ...
                        print(
                            f"   ✅ Found complete .next build directory - using direct binary path"
                        )
                    else:
                        print(
                            f"   ⚠️  Incomplete .next build - missing server directory"
                        )
                        print(f"   🔄 Will attempt to rebuild during startup")
                        start_command = (
                            "npm run build && node_modules/next/dist/bin/next start"
                        )
                else:
                ...
                try:
                    with open(os.path.join(final_dir, "package.json"), "r") as f:
                        ...
                        if "next" in dependencies:
                            app_type = "nextjs"
                            print(
                                f"   🔍 Detected Next.js application via dependencies"
                            )
                            ...
                            if os.path.exists(os.path.join(final_dir, ".next")):
                                ...
                                start_command = "node_modules/next/dist/bin/next start"
                                print(
                                    f"   ✅ Found .next directory - using direct binary path"
                                )
                            else:
                                start_command = "npm run build && node_modules/next/dist/bin/next start"
                                print(
                                    f"   🔨 No .next directory - will build then start with direct binary"
                                )
                            ...
                            print(
                                f"   ⚠️  package.json exists but no start/dev script found"
                            )
            ...
            if not start_command:
                ...
                if not start_command:
                    ...
                    print(
                        f"   📋 Directory contents: {os.listdir(final_dir) if os.path.exists(final_dir) else 'Directory not found'}"
                    )
                    print(
                        f"   💡 For Next.js apps, ensure .next directory exists (run 'npm run build')"
                    )
                    print(
                        f"   💡 For Node.js apps, ensure you have index.js, server.js, or app.js"
                    )
                    print(
                        f"   💡 For npm apps, ensure package.json has a 'start' script"
                    )
                    return False
...
"""
            ...
            try:
                ...
                return self.create_readonly_process_manager(
                    site_name, final_dir, app_port, start_command
                )

            subprocess.run(["systemctl", "daemon-reload"], capture_output=True)
            subprocess.run(
                ["systemctl", "enable", f"nodejs-{site_name}"], capture_output=True
            )
            ...
            if not self.readonly_filesystem:
                ...
                return self.create_readonly_process_manager(
                    ...
                    final_dir,
                    app_port,
                    start_command if "start_command" in locals() else None,
                )
            return False
        ...
        try:
            ...
            if not self.ensure_nginx_installed():
                if not self.readonly_filesystem:
                    ...
                    print(
                        "⚠️  Nginx verification failed but continuing (read-only mode)"
                    )
...
"""
            ...
            print(
                '        -d \'{"domain_name": "yourdomain.com", "port": 80, "site_type": "static"}\''
            )
            print()
            ...
            print("        -H 'Content-Type: application/json' \\")
            print(
                '        -d \'{"name": "myapp", "files": {...}, "deployConfig": {"port": 3000}}\''
            )
    ...
    def setup_npm_environment(self):
        ...
        try:
            ...
            npm_dirs = ["/tmp/npm", "/tmp/npm-cache", "/tmp/npm-global"]

            for npm_dir in npm_dirs:
                if not self.create_directory_with_permissions(
                    npm_dir, "www-data", 0o755
                ):
            ...
            for cmd in npm_config_commands:
                ...
                if result.returncode == 0:
                    ...
                    result2 = subprocess.run(
                        legacy_cmd, shell=True, capture_output=True, text=True
                    )
                    if result2.returncode == 0:
                        success_count += 1

            print(
                f"   ✅ {success_count}/{len(npm_config_commands)} npm configurations applied"
            )
            return True
    ...
    def run_npm_install_safely(self, temp_dir, deploy_env, npm_cache_dir):
        ...
        install_strategies = [
            ...
            ["npm", "ci", "--cache", npm_cache_dir, "--silent"],
            ["npm", "install"],
        ]
    ...
    def test_database_access(self):
        ...
        try:
            ...
            cursor.execute(
                "INSERT OR IGNORE INTO deployment_logs (domain_name, action, status, message) VALUES (?, ?, ?, ?)",
                ("test.example.com", "test", "success", "Database test"),
            )
            conn.commit()

            cursor.execute(
                "SELECT COUNT(*) FROM deployment_logs WHERE domain_name = 'test.example.com'"
            )
            count = cursor.fetchone()[0]

            cursor.execute(
                "DELETE FROM deployment_logs WHERE domain_name = 'test.example.com'"
            )
            conn.commit()
            conn.close()
...
"""
    ...
    def deploy_domain(self, domain_name, port, site_type="static"):
        ...
        try:
            ...
            if not self.create_directory_with_permissions(
                domain_path, "www-data", 0o755
            ):
                return False
            if not self.create_directory_with_permissions(
                public_path, "www-data", 0o755
            ):
                return False
        ...
        <div class="info">
            ...
            if not self.readonly_filesystem:
                ...
                try:
                    ...
                    result = subprocess.run(
                        ["sudo", "ln", "-s", nginx_file, enabled_file],
                        capture_output=True,
                        text=True,
                    )
                    if result.returncode == 0:
            ...
            try:
                ...
                if conn:
                    ...
                    print(
                        "   ⚠️  Database connection failed - domain deployed but not tracked"
                    )
            ...
            if not self.readonly_filesystem:
                print(
                    f"🔒 Add SSL: curl -X POST http://localhost:5000/api/domains/{domain_name}/ssl"
                )
            if site_type != "static":
                print(
                    f"⚡ Remember to start your {site_type} application on port {port}"
                )

            if self.readonly_filesystem:
                print(
                    "🔒 Read-only filesystem mode - some features limited but core functionality available"
                )
    ...
    def setup_nginx_proxy(self, site_name, port):
        ...
        try:
            ...
            if self.readonly_filesystem:
                ...
                print(
                    f"   💡 Manual nginx config needed for {site_name} -> localhost:{port}"
                )
                return True
...
"""
            ...
            try:
                ...
                try:
                    ...
                    subprocess.run(
                        ["sudo", "ln", "-s", config_path, enabled_path], check=True
                    )
            ...
            print(
                f"✅ Nginx proxy configured successfully: {site_name} -> localhost:{port}"
            )
            return True
    ...
    def add_ssl(self, domain_name):
        ...
        try:
            if self.readonly_filesystem:
                print(
                    f"🔒 Read-only mode: SSL certificate setup skipped for {domain_name}"
                )
                print(
                    "   💡 SSL certificates cannot be managed in read-only filesystem mode"
                )
                return False
            ...
            if conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT domain_name FROM domains WHERE domain_name = ? AND status = 'active'",
                    (domain_name,),
                )
            ...
            result = subprocess.run(
                certbot_command, shell=True, capture_output=True, text=True
            )
            if result.returncode == 0:
                conn = self.get_database_connection()
                if conn:
    ...
    def list_domains(self):
        ...
        try:
            ...
            for domain in domains:
                ...
                url = (
                    f"https://{domain_name}" if ssl_enabled else f"http://{domain_name}"
                )

                print(
                    f"{ssl_icon} {type_icon} {domain_name:30} | Port: {port:5} | Type: {site_type:8} | {created_at[:10]}"
                )
                print(f"    📍 {url}")
    ...
    def remove_domain(self, domain_name):
        ...
        try:
            ...
            if conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE domains SET status = 'removed' WHERE domain_name = ?",
                    (domain_name,),
                )
                cursor.execute(
                    """
...
class SimpleAPI:
    ...
    def setup_routes(self):
            ...
            try:
                ...
                try:
                    ...
                    if conn:
                        ...
                        cursor.execute(
                            "SELECT COUNT(*) FROM domains WHERE ssl_enabled = 1"
                        )
        ...
        def deploy_nodejs_app():
            ...
            try:
                ...
                if not site_name or not project_files:
                    return jsonify(
                        {"success": False, "error": "Missing site name or files"}
                    )
                ...
                deploy_env, npm_cache_dir, npm_prefix_dir = (
                    self.manager.get_npm_environment_for_deployment(
                ...
                if self.manager.readonly_filesystem:
                    ...
                    pm_success = self.manager.create_readonly_process_manager(
                        site_name, final_dir, app_port
                    )
                    if pm_success:
                        process_manager = "readonly-simple"
                    ...
                    systemd_success = self.manager.create_systemd_app_service(
                        site_name, final_dir, app_port
                    )
                    if systemd_success:
                        ...
                        return jsonify(
                            {"success": False, "error": "Systemd deployment failed"}
                        )
                ...
                if not nginx_success and not self.manager.readonly_filesystem:
                    ...
                    return jsonify(
                        {"success": False, "error": "Nginx configuration failed"}
                    )
                ...
                if self.manager.readonly_filesystem:
                    response_data["notes"] = (
                        "Deployed in read-only filesystem mode with limited features"
                    )
                ...
                return jsonify(
                    {
                        "success": False,
                        "error": "Deployment timeout - process took too long",
                    }
                )
            except Exception as e:
        ...
        def deploy_domain():
            ...
            try:
                ...
                if success:
                    ...
                    if self.manager.readonly_filesystem:
                        warnings.append(
                            "Read-only filesystem detected - SSL and some nginx features limited"
                        )
        ...
        def remove_domain(self, domain_name):
            """Remove a domain and stop any associated running applications"""
            try:
                print(f"🗑️  Removing {domain_name}...")

                # Check if there's a running Node.js app for this domain and stop it
                print("🔍 Checking for running Node.js applications...")

                # Check in read-only mode location
                if self.readonly_filesystem:
                    config_file = f"/tmp/nodejs-apps/{domain_name}/deployment.json"
                else:
                    config_file = f"/var/lib/hosting-apps/{domain_name}/deployment.json"
                ...
                if os.path.exists(config_file):
                    try:
                        ...
                        process_manager = config.get("process_manager", "unknown")
                        print(
                            f"   📱 Found running app with {process_manager} process manager"
                        )

                        # Stop the application
                        if process_manager == "readonly-simple":
                            if self.stop_readonly_app(domain_name):
                                print(
                                    f"   ✅ Stopped read-only managed app: {domain_name}"
                                )
                            else:
                                print(
                                    f"   ⚠️  Failed to stop read-only managed app: {domain_name}"
                                )
                        elif process_manager == "systemd":
                            if self.stop_systemd_app(domain_name):
                                print(
                                    f"   ✅ Stopped systemd service: nodejs-{domain_name}"
                                )
                            else:
                                print(
                                    f"   ⚠️  Failed to stop systemd service: nodejs-{domain_name}"
                                )

                        # Remove systemd service file if it exists
                        if (
                            process_manager == "systemd"
                            and not self.readonly_filesystem
                        ):
                            service_file = (
                                f"/etc/systemd/system/nodejs-{domain_name}.service"
                            )
                            if os.path.exists(service_file):
                                try:
                                    ...
                                    print(
                                        f"   ✅ Removed systemd service file: {service_file}"
                                    )
                                except Exception as e:
                                    print(
                                        f"   ⚠️  Could not remove systemd service file: {e}"
                                    )

                        # Clean up deployment configuration directory
                        config_dir = os.path.dirname(config_file)
                        if os.path.exists(config_dir):
                            shutil.rmtree(config_dir, ignore_errors=True)
                            print(f"   ✅ Cleaned up deployment config: {config_dir}")

                    except Exception as e:
                        print(f"   ⚠️  Error stopping application: {e}")
                else:
                    print("   ℹ️  No running Node.js application found")
                ...
                if not self.readonly_filesystem:
                    ...
                    if os.path.exists(nginx_config):
                        os.remove(nginx_config)
                        print(f"   ✅ Removed nginx config: {nginx_config}")
                else:
                    print("   🔒 Read-only mode: nginx configs not removed")
                ...
                if conn:
                    ...
                    cursor.execute(
                        """
                        INSERT INTO deployment_logs (domain_name, action, status, message)
                        VALUES (?, 'remove', 'success', 'Domain and associated applications removed successfully')
                    """,
                        (domain_name,),
                    )
                    conn.commit()
                    conn.close()
                    print(f"   ✅ Database updated: {domain_name} marked as removed")

                # Clean up application files directory if it exists
                app_files_dir = f"{CONFIG['web_root']}/{domain_name}"
                if os.path.exists(app_files_dir):
                    try:
                        shutil.rmtree(app_files_dir, ignore_errors=True)
                        print(f"   ✅ Cleaned up application files: {app_files_dir}")
                    except Exception as e:
                        print(f"   ⚠️  Could not remove application files: {e}")
                ...
                if not self.readonly_filesystem:
                    if self.test_nginx_config_safe():
                        ...
                        print("   ⚠️  Nginx config test failed after removal")

                print(
                    f"✅ {domain_name} and all associated applications removed successfully!"
                )
                print(f"🔄 Domain {domain_name} is now available for reuse")
                return True
    ...
    def run(self, host="0.0.0.0", port=5000, debug=False):
        """Start API server"""
        print(
            f"🚀 Starting Simple Multi-Domain Hosting API v2.6 on http://{host}:{port}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Simple Multi-Domain Hosting v2.6 - Read-Only Filesystem Edition"
    )
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Complete system setup with read-only filesystem support",
    )
    ...
    parser.add_argument(
        "command", nargs="?", help="Command: deploy, ssl, remove, list, status"
    )
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="static",
        help="Site type: static, api, node, app",
    )
        ...
        if args.setup:
            ...
            if success:
                ...
                if not manager.readonly_filesystem:
                    ...
                    print(
                        "🚀 Start the API manually: sudo python3 simple-hosting.py --api"
                    )
                print("\n💡 Usage Examples:")
                print(
                    "   Deploy domain: curl -X POST http://localhost:5000/api/domains \\"
                )
                print("     -H 'Content-Type: application/json' \\")
                print(
                    '     -d \'{"domain_name": "test.com", "port": 80, "site_type": "static"}\''
                )
                print(
                    "   Deploy Node.js app: curl -X POST http://localhost:5000/api/deploy/nodejs \\"
                )
                print("     -H 'Content-Type: application/json' \\")
                print(
                    '     -d \'{"name": "myapp", "files": {...}, "deployConfig": {"port": 3000}}\''
            ...
            if not manager.readonly_filesystem:
                api_status = subprocess.run(
                    ["systemctl", "is-active", "hosting-api"],
                    capture_output=True,
                    text=True,
                )
                api_running = api_status.returncode == 0
                print(
                    f"⚡ API Service: {'✅ Running' if api_running else '❌ Stopped'}"
                )
            else:
            ...
            if not args.domain or not args.port:
                print(
                    "❌ Usage: python3 simple-hosting.py deploy <domain> <port> [type]"
                )
            ...
            print("\n📖 Setup Commands:")
            print(
                "   --setup                              Complete system setup with read-only support"
            )
            ...
            print(
                "   ssl <domain>                         Add SSL (disabled in read-only mode)"
            )
            ...
            print(
                "   ⚠️  SSL certificate management disabled (requires file system writes)"
            )
            print(
                "   ⚠️  Some nginx features limited (manual configuration may be needed)"
            )
