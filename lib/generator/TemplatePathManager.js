...
export class TemplatePathManager {
    ...
    // Method to get conditional paths based on enabled pages
    getConditionalPaths(config) {
        const conditionalPaths = {};
        const basePaths = this.getTemplatePaths(config.projectType);
        ...
        if (config.pages) {
            Object.entries(config.pages).forEach(([pageKey, pageConfig]) => {
                if (pageConfig.enabled && pageKey !== 'home') {
                    const templatePath = `${config.projectType}/app/${pageKey}/page.js.template`;
                    const outputPath = `app/${pageKey}/page.js`;

                    // Check if template exists for this project type, otherwise use base
